...
def agregar_disclaimer(pdf):
    ...
    pdf.multi_cell(0, 3, txt="Esta simulación se emite con fines orientativos. Los valores definitivos surgirán de las proformas oficiales de los escribanos y liquidaciones finales de impuestos vigentes al momento de la firma.", align="C")
...
def generar_pdf_comparativo(casos, tc, mni_val):
    ...
    for caso in casos:
        ...
        items = [
            ["Honorarios Inmobiliarios", f"{caso['res']['comision']:,.2f}", f"{caso['com_pct']}% s/ Real"],
            ["IVA s/ Honorarios", f"{caso['res']['iva_com']:,.2f}", "21% Profesional"],
            ["Honorarios Escribanía", f"{caso['res']['hono_esc']:,.2f}", f"{caso['esc_pct']}% s/ Escritura"],
            ["IVA s/ Escribanía", f"{caso['res']['iva_esc']:,.2f}", "21% Profesional"],
            ["Impuesto Sellos (50%)", f"{caso['res']['sellos']:,.2f}", "CABA 2026"]
        ]
        for item in items:
    ...
    if len(casos) > 1:
        ...
        filas = [
            ...
            ["Gastos Totales (USD)", f"{casos[0]['res']['total_gastos']:,.2f}", f"{casos[1]['res']['total_gastos']:,.2f}"],
...
com_a = col_a1.number_input("% Comisión Inmob. (A)", value=3.0)
...
if st.session_state.mostrar_caso_b:
    ...
    com_b = col_b1.number_input("% Comisión Inmob. (B)", value=com_a)
...
st.divider()
st.download_button("📄 Descargar Informe Comparativo Completo (PDF)", generar_pdf_comparativo(lista_casos, tc, mni), f"Simulacion_Llamedo_{rol_a}.pdf", "application/pdf")
