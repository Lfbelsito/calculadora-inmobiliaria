...
# CSS Mejorado para forzar colores institucionales en cualquier modo
st.markdown("""
    <style>
    /* Forzamos el fondo crema en toda la app */
    .stApp { background-color: #F9F7F2 !important; }
    
    /* Forzamos el color de texto Verde Oscuro en etiquetas y títulos */
    h1, h2, h3, label, .stMarkdown p { 
        color: #0B3D2E !important; 
        font-family: 'Georgia', serif; 
    }
    
    /* Estilos específicos para la Barra Lateral (Siempre Verde) */
    ...
    /* Inputs: Fondo blanco y texto verde oscuro para legibilidad */
    input, div[data-baseweb="select"] > div { 
        background-color: white !important; 
        color: #0B3D2E !important; 
    }
    
    /* Botones con estilo Llamedo */
    .stButton>button { 
...
def calcular_operacion(p_real, p_esc, com_pct, esc_pct, tipo, tc, mni):
    ...
    return {
        ...
        'sellos': sellos_parte_usd, 'total_gastos': comision + iva_com + hono_esc + iva_esc + sellos_parte_usd
    }

# 4. FUNCIÓN PARA EL DISCLAIMER EN CADA HOJA 
def agregar_disclaimer(pdf):
    ...
    pdf.multi_cell(0, 3, txt="Esta simulación se emite con fines orientativos e informativos. "
                             ...
                             "y de las liquidaciones finales de impuestos vigentes al momento de la firma.", align="C")
...
def generar_pdf_comparativo(casos, tc, mni_val):
    ...
    for caso in casos:
        ...
        for item in items:
            pdf.cell(85, 8, f" {item[0]}", 1); pdf.cell(45, 8, f"{item[1]}", 1, 0, 'R'); pdf.cell(60, 8, f" {item[2]}", 1, 1)
    ...
    # Página 3: Resumen Comparativo (Solo si hay Caso B)
    if len(casos) > 1:
        pdf.add_page()
        ...
        pdf.cell(0, 15, "RESUMEN COMPARATIVO", ln=True, align="C")
        pdf.ln(10)
...
lista_casos = [{'nombre': 'A', 'rol': rol_a, 'p_real': p_real_a, 'p_esc': p_esc_a, 'tipo': tipo_a, 'direccion': dir_a, 'unidad': "", 'res': res_a}]
...
if st.session_state.mostrar_caso_b:
    ...
    lista_casos.append({'nombre': 'B', 'rol': rol_b, 'p_real': p_real_b, 'p_esc': p_esc_b, 'tipo': tipo_b, 'direccion': dir_b, 'unidad': "", 'res': res_b})
